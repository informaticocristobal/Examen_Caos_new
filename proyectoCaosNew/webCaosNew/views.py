...
def index (request):
    categorias = Categoria.objects.all()
    noticia=Noticia.objects.all().filter(publicar=True).order_by("-titular_noticia")[:3]
    print(noticia)
    contexto={'categorias':categorias,'noticias':noticia}
    return render(request,"index.html",contexto)

def galeria(request):
    noticias=Noticia.objects.all().filter(publicar=True).order_by("-titular_noticia")[:3]
    categorias = Categoria.objects.all()
